...
catalog = Table(names = ('Object', 'X', 'Y', 'RA', 'Dec', 'pol_median','pol_width','median_sigp','Theta_median','Theta_width','Theta_sigp_median','mag_median','mag_width','sig_mag_median','Nos_Epochs','p_ratio','t_ratio', 'm_ratio','potential' ),dtype=('S32','f16', 'f16', 'f16', 'f16','f16', 'f16', 'f16','f16','f16','f16','f16','f16','f16','f16','f16','f16','f16','S16'))

candidates = Table(names=('Object', 'X', 'Y', 'RA', 'Dec', 'pol_median','pol_width','median_sigp','theta_median','theta_width','theta_sigp_median','mag_median','mag_width','sig_mag_median','Nos_Epochs','p_ratio','t_ratio','m_ratio'), dtype=('S32','f16','f16','f16','f16','f16', 'f16','f16','f16','f16','f16','f16','f16','f16','f16','f16','f16','f16'))


Catalog_Dir = "/media/user/Seagate Backup Plus Drive/DATA/RoboPol/Catalog"
...
for k in range(len(table_files)):
    ...
    if len(tabe['p']) >=5:                    #minimum no of obsevations required
        ...
        if pol_median <= 0.005: 
            candidates.add_row([ID,X, Y, RA, Dec, pol_median, pol_width, median_sigp, theta_median ,theta_width ,theta_sig_median,mag_median, mag_width, median_sig_mag, len(tabe['p']), P_Ratio,T_Ratio, M_Ratio])
            print '********************************got a zero polarization candidate************************************************', ID 
...
candidates.write(Candidate_Dir + "/" + 'zero_polarization_candidates.dat', format = 'ascii')
